...
| ID | Name | Payload bytes | Frame bytes | Current source/rate |
|---:|---|---:|---:|---|
| `0x01` | Sensor | 21 | 23 | USB, 10 Hz |
| `0x02` | Estimated state | 62 | 64 | USB and LoRa, 1 Hz |
| `0x03` | Camera telemetry | 17 | 19 | Defined, not currently produced |
| `0x05` | Command | 8 | 10 | Received from USB/LoRa and bridged to UART2 |
| `0xE0` | Witness debug | 101 | 103 | USB, 10 Hz after debug enable |
| `0xE1` | IRIS debug | 12 | 14 | Defined, not currently produced |
| `0xFF` | Heartbeat | 6 | 8 | USB, 1 Hz |
| `0xF0` | Ground-station status | 11 | 13 | USB, 1 Hz; ground station only |
...
### `0xF0` Ground-station status packet, 13 bytes

This packet is generated by the ground-station bridge and sent only to the
host computer over USB. It is never transmitted over LoRa.

| Frame offset | Size | Field |
|---:|---:|---|
| 0 | 1 | Packet ID `0xF0` |
| 1 | 1 | RSSI of the last valid LoRa packet, signed dBm |
| 2 | 4 | Ground-station uptime, milliseconds, uint32 BE |
| 6 | 6 | Reserved bytes, transmitted as zero |
| 12 | 1 | EOF `0x0A` |
...
Witness transmits the complete 64-byte `0x02` frame once per second. Between
transmissions the radio is returned to continuous receive mode. DIO1 indicates
RX completion, and received data is read by the sensor task so SPI3 access does
not race the sensors or radio transmitter.
...
forwarded unchanged to UART2. SX1262 packets with a CRC error are discarded.
...
6. Send exactly ten LoRa payload bytes; do not send ASCII hexadecimal text over
   LoRa.
7. Expect Witness state telemetry as one complete 64-byte LoRa payload at 1 Hz.
